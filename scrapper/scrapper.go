...
func Scrape(term string) {
	...
	// 마지막 페이지 번호 얻기
	totalPage := getMaxPageNumber(baseURL)
	fmt.Println("total pages count :", totalPage)
  

	// 전체 페이지 url 획득
	for i := 0; i < totalPage; i++ {
		go makeData(getPageURL(baseURL,i), c)
	}  
...
func makeChromeDriver() *selenium.Service{
	...
	service, err := selenium.NewChromeDriverService(path+"/chromedriver", 8080, opts...)
	if err != nil {
		panic(err)
	}
	return service
...
// getPageMax 총 페이지 확인 
func getMaxPageNumber(baseURL string) int {
	wd := makeBrowser()
	defer wd.Quit()
	openPage(wd, baseURL)
...
func makeData(URL string, mainC chan<- []extractedJob)  {
	...
	scrollDown(wd, "window.scrollTo({top: document.body.scrollHeight, behavior: 'smooth'});")
